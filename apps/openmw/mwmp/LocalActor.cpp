...
{
    LOG_MESSAGE_SIMPLE(MWMPLog::LOG_VERBOSE, "Sending ID_ACTOR_CELL_CHANGE about %s %i-%i in cell %s to server",
                       refId.c_str(), refNum, mpNum, cell.getDescription().c_str());

    LOG_APPEND(MWMPLog::LOG_VERBOSE, "- Moved to cell %s", ptr.getCell()->getCell()->getDescription().c_str());
...
void LocalActor::updateStatsDynamic(bool forceUpdate)
{
        ...
        needUpdate(oldFatigue, fatigue, 7))
    {
        ...
        if (creatureStats.mDead && !wasDead)
        {
            ...
            LOG_MESSAGE_SIMPLE(MWMPLog::LOG_INFO, "Sending ID_ACTOR_DEATH about %s %i-%i in cell %s to server",
                refId.c_str(), refNum, mpNum, cell.getDescription().c_str());
